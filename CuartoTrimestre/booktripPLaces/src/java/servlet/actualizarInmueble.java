 ...
 */
@WebServlet(name = "actualizarInmueble", urlPatterns = {"/actualizarInmueble"})
public class actualizarInmueble extends HttpServlet {
...
String BanderaInmueble = request.getParameter("BanderaActualiza");
// Declaración variables y objetos Modelo y Controlador.
inmueblesDAO miinmuebleDAO = new inmueblesDAO(); // Controlador
inmuebles miInmueble = new inmuebles(); // Modelo
...
String respuestaActualizada = miinmuebleDAO.actualizarInmueble(miInmueble);
System.out.println("Res " + respuestaActualizada);
System.out.println("Res " + respuestaActualizada.length());
if (respuestaActualizada.length() == 0) {
out.println("<script type=\"text/javascript\">");
out.println("alert('" + "Actualización Realizada." + "');");

// Se agrega el redreccionamiento la url para visualice la tabla.
out.println("window.location.href = '/booktripPlaces/inmueble.jsp';");
out.println("</script>");
} else {
out.println("<script type=\"text/javascript\">");
