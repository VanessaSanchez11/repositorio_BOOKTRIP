 ...
 * @author 57321
 */
@WebServlet(name = "consultarDepartamentos", urlPatterns = {"/consultarDepartamentos"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           String Accion = request.getParameter("Actualizar");
            ...
            Departa = departamento.ConsultarDepartamento("nombre");

            if (Accion != null) {

                if (Departa != null) {

                    response.sendRedirect("/booktripP/vista/Dashboard/indexListaDepartamento.jsp?idUsuario="+ Departa.getNombre()+ "&"
       
                            + "password=" + Departa.getNombre() + "&"
                            + "Vista=" + "Actualizar" + "&");
                    ...
                    out.println("alert('" + "No se ha podido relizar la consulta." + "\n"
                            + "Por favor verificar el nombre del departamento " +  "');");
                    out.println("</script>");
                }
            } else {
                if ( Departa != null) {
                    String respuestaRegistrada = departamento.EliminarDepartamento(Departa);
