<div class="d-sm-flex align-items-center justify-content-between mb-4">

    <a href="#" class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm"> Lista de Municipios</a>
    <a href="#" class="d-none d-sm-inline-block btn btn-sm btn-success shadow-sm"> Consultar Municipios</a>
    <a href="#" class="d-none d-sm-inline-block btn btn-sm btn-warning shadow-sm"> Actualizar Municipios</a>
    <a href="#" class="d-none d-sm-inline-block btn btn-sm btn-danger shadow-sm"> Eliminar Municipios</a>

</div>

    <body>

        <h1 class="parte3">REGISTRA EL MUNICIPIO</h1>
        <br>
        <div class="form-group col-sm-5 ">
            <br>
            <label class="label">SELECIONA EL DEPARTAMENTO</label>
            <select>
                <option>ELIGE UNA OPCION</option>
                <option>Cundinamarca</option>
                <option>Tolima</option>
                <option>Boyaca</option> 
                <option>Guajira</option> 
                <option>Nariño</option> 
                <option>Caldas</option> 
                <option>Antioquia</option>
            </select>
            <br>
            <br>

            <label for="name" > ESCRIBE EL MUNICIPIO </label>
            
                <input type="text" id="name" name="user_name">
                <br>
                <br>
            <TABLE BORDER=2 class="ubicacion">
               <TR><TD>
                <MARQUEE> ¡PUEDES UBICARTE!</MARQUEE>
                </TD></TR>
            </TABLE>
            <br>
            <br>
            <br>
            <br>
            <div class="mapa">
                <iframe src="https://www.google.com/maps/embed?pb=!1m14!1m12!1m3!1d254508.51141489705!2d-74.107807!3d4.64829755!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!5e0!3m2!1ses!2sco!4v1595012598872!5m2!1ses!2sco"
                        width="500" height="350" frameborder="0" style="border:2;" allowfullscreen="" aria-hidden="false" tabindex="0"></iframe>
            </div>
            <br>
            <br>
            <button type="button" class="btn btn-primary btn-sm botonRegistro mb-4">Registrar</button>
        </div>


